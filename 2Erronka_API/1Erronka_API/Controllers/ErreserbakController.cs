...
namespace _1Erronka_API.Controllers
{
    ...
    public class ErreserbakController : ControllerBase
    {
        ...
        public ErreserbakController(ErreserbaRepository repo, EskariaRepository eskariaRepo, ProduktuaRepository produktuaRepo, ProduktuaOsagaiaRepository produktuaOsagaiaRepo, OsagaiaRepository osagaiaRepo, IWebHostEnvironment env)
        {
            _repo = repo;
        ...
        public IActionResult GetAll()
        {
            ...
            var dtoList = erreserbak.Select(e => new ErreserbaDto
            {
                ...
                FakturaRuta = string.IsNullOrEmpty(e.FakturaRuta)
                    ? e.FakturaRuta
                    : (e.FakturaRuta.StartsWith("/") ? e.FakturaRuta : $"/tiketak/{e.FakturaRuta}"),
        ...
        {
            string? fakturaFitxategia = null;

            try
            {
                using (var readSession = _repo.OpenSession())
                {
                    var erreserbaPdf = readSession.Get<Erreserba>(dto.ErreserbaId);
                    if (erreserbaPdf == null) return NotFound();

                    var produktuak = _repo.LortuProduktuakErreserbarako(dto.ErreserbaId, readSession);
                    ...
                    fakturaFitxategia = Path.GetFileName(fakturaRuta);
                }
                ...
                for (int attempt = 1; attempt <= maxAttempts; attempt++)
                {
                    try
                    {
                        ...
                        erreserba.FakturaRuta = fakturaFitxategia!;
                    ...
                    catch (Exception ex) when (attempt < maxAttempts && IsTransientMySqlConcurrency(ex))
                    {
                        Thread.Sleep(50 * attempt);
                    }
                }

                throw new Exception("Ezin izan da ordainketa burutu. Saiatu berriro.");
            }
            catch (Exception ex)
            {
        ...
        private static bool IsTransientMySqlConcurrency(Exception ex)
        {
            for (Exception? cur = ex; cur != null; cur = cur.InnerException)
            {
                ...
                if (typeName.Contains("MySqlException", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (numberProp?.GetValue(cur) is int num && (num == 1213 || num == 1205))
                    {
                        return true;
                    }

                    var msg = cur.Message ?? "";
                    if (msg.Contains("Deadlock found", StringComparison.OrdinalIgnoreCase) ||
                        msg.Contains("Lock wait timeout", StringComparison.OrdinalIgnoreCase))
                    {
                        return true;
                    }
                }

                if (cur is HibernateException)
                {
                    var msg = cur.Message ?? "";
            ...
            string ordainketaModua)
        {
            ...
            string izenFitxategia = $"T_{erreserba.Id}_{uniq}.pdf";
        ...
        public IActionResult DeskargatuTicket(int id)
        {
            ...
            var fakturaRuta = erreserba.FakturaRuta.Trim();
            var fileName = Path.GetFileName(fakturaRuta);
